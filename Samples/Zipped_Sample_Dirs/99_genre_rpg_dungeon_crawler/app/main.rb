...
class DungeonHandler
    ...
    def render  
        ...
        #render characters
        state.skeleton.name = "skeleton/skeleton"
        ...
        if state.render_fire_interact
            outputs.sprites << [flame_x, flame_y, flame_size, flame_size, 'sprites/added/icons/Flame.png', 0, easeIt(state, 0, 255, state.click_time+10)]
        #transition the alpha of the fire interact UI out
        elsif state.render_fire_interact_end
            outputs.sprites << [flame_x, flame_y, flame_size, flame_size, 'sprites/added/icons/Flame.png', 0, easeIt(state, 255, 0, state.click_time)]
        end
    ...
    def renderInteract alpha
        outputs.sprites << [state.interact_border.x-30, state.interact_border.y-50, 380, 330, "sprites/added/icons/Interact_UI.png", 0, alpha]

        # for each interact spot, create a sprite
        outputs.sprites << state.interact_area.map do |a|
            [a.x, a.y, a.w, a.h, 'sprites/square/white.png', 0, alpha ]
        end

        #draw interact buttons here:
        outputs.sprites << [state.interact.accept.x, state.interact.accept.y, state.interact.accept_reject_w_h, state.interact.accept_reject_w_h, 'sprites/added/icons/green_check.png', 0, alpha]
        outputs.sprites << [state.interact.reject.x, state.interact.reject.y, state.interact.accept_reject_w_h, state.interact.accept_reject_w_h, 'sprites/added/icons/red_x.png', 0, alpha]
    end
    ...
    def renderInteractButton alpha
        ...
        outputs.labels  << [state.interact.start.x+15, state.interact.start.y+40, 'Start Interaction', 2, 0, 0, 0, 0, alpha]
    end 
    ...
    def render_background
        outputs.sprites << [0, 0, 1280, 720, 'sprites/added/parallax_dragon/background.png']

        #scroll_point_at = state.tick_count
        scroll_point_at ||= 0
        scroll_point_at = 1280 * (easeSpline state, outputs, 30)

        outputs.sprites << scrolling_background(scroll_point_at, 'sprites/added/parallax_dragon/parallax_back.png', 0.30)
        outputs.sprites << scrolling_background(scroll_point_at, 'sprites/added/parallax_dragon/parallax_middle.png', 0.50)
        outputs.sprites << scrolling_background(scroll_point_at, 'sprites/added/parallax_dragon/parallax_front.png', 0.70)
    end
