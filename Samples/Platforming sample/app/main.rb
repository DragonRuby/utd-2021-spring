class Game
  ...
  def tick
    ...
    player.time_since_walljump += 1
    player.time_since_leftwall   += 1
    player.time_since_rightwall  += 1
    player.time_since_grounded   += 1
    input_edit_map
    
    input_player
    calc_player
    
    if (player.touching == 1 || player.touching == 2) && player.grounded == 0
      state.drag = -0.4 
    else
    state.drag = -0.15
    end
  ...
  def defaults
    ...
    state.player.touching       ||= 0 #0 equals touching(white), 1 equals left kick (violet), 2 equals right kick (red), 3 equals midair (green)
    state.player.grounded       ||= 0
    state.player.time_since_walljump    ||= 0
    state.player.time_since_leftwall    ||= 0
    state.player.time_since_rightwall   ||= 0
    state.player.time_since_grounded    ||= 0
    state.player.stairs                 ||= 0

    calc_player_rect if !state.player.rect
  end

  def render
    elka =  [160, 256,    8,     8, "sprites/square/gray.png",    0]
    elke =  [168, 264,    8,     8, "sprites/square/gray.png",    0]
    elki =  [176, 272,    8,     8, "sprites/square/gray.png",    0]
    elko =  [184, 280,    8,     8, "sprites/square/gray.png",    0]
    if player.rect.intersect_rect? elka 
        player.y=264
        player.dy=0
        gravity=0
        player.stairs=1
    ...
    if player.rect.intersect_rect? elke
        player.y=272
        player.dy=0
        gravity=0
        player.stairs=1
    ...
    if player.rect.intersect_rect? elki
        player.y=280
        player.dy=0
        gravity=0
        player.stairs=1
    ...
    if player.rect.intersect_rect? elko
        player.y=288
        player.dy=0
        gravity=0
        player.stairs=1
    ...
    alka =  [288, 288,    8,     8, "sprites/square/gray.png",    0]
    alke =  [296, 296,    8,     8, "sprites/square/gray.png",    0]
    alki =  [304, 304,    8,     8, "sprites/square/gray.png",    0]
    alko =  [312, 312,    8,     8, "sprites/square/gray.png",    0]
    if player.rect.intersect_rect? alka 
        player.y=296
        player.dy=0
        gravity=0
        player.stairs=1
    ...
    if player.rect.intersect_rect? alke
        player.y=304
        player.dy=0
        gravity=0
        player.stairs=1
    ...
    if player.rect.intersect_rect? alki
        player.y=312
        player.dy=0
        gravity=0
        player.stairs=1
    ...
    if player.rect.intersect_rect? alko
        player.y=320
        player.dy=0
        gravity=0
        player.stairs=1
    ...
    args.outputs.sprites << elka
    args.outputs.sprites << elke
    args.outputs.sprites << elki
    args.outputs.sprites << elko

    args.outputs.sprites << alka
    args.outputs.sprites << alke
    args.outputs.sprites << alki
    args.outputs.sprites << alko
    #outputs.labels << [10, 10.from_top, "tile: click to add a tile, hold X key and click to delete a tile."]
    #outputs.labels << [10, 35.from_top, "move: use left and right to move, space to jump or walljump on a wall, down and space to jump down."]
    #outputs.labels << [10, 55.from_top, "      You can jump through or jump down through tiles with a height of 1."]
    outputs.labels << [10, 35.from_top, " <= #{player.y}"]
    
        
    
    outputs.background_color = [0, 0, 0]
    outputs.sprites << tiles.map(&:sprite)
    if player.touching == 3
  ...
  def input_player
    if player.time_since_walljump > 14
      player.dx += inputs.left_right*0.4
    end
    ...
    elsif inputs.keyboard.key_down.space && (player.grounded == 1 || player.time_since_grounded < 5)
      player.dy             = player.jump_power
      player.jumped_at      = state.tick_count
      player.jumped_down_at = 0
    elsif inputs.keyboard.key_down.space && (player.touching == 1 || player.time_since_leftwall<20) && player.grounded == 0 
      player.dy             = player.jump_power
      player.dx             = 3*player.jump_power
      player.jumped_at      = state.tick_count
      player.jumped_down_at = 0
      player.time_since_walljump = 0
    elsif inputs.keyboard.key_down.space && (player.touching == 2 || player.time_since_rightwall<20) && player.grounded == 0 
      player.dy             = player.jump_power
      player.dx             = -3*player.jump_power
      player.jumped_at      = state.tick_count
      player.jumped_down_at = 0
      player.time_since_walljump = 0
    elsif inputs.up && (player.touching == 1 || player.touching == 2)
      player.dy += 0.5
    end
  end
  ...
  elka =  [160, 256,    8,     8, "sprites/square/gray.png",    0]
    ...
    outputs.sprites << (player.rect.merge path: 'sprites/square/white.png')
    player.touching= 0
    player.grounded=1
  ...
  def calc_left
    tiles_left = find_tiles { |t| t.rect.right >= player.rect.left-20 }
    if find_colliding_tile tiles_left, (player.rect.merge x: player.rect.x)
    #outputs.labels << [10, 35.from_top, "grumby"]
    end
    ...
    player.dx = 0
    outputs.sprites << (player.rect.merge path: 'sprites/square/violet.png')
    player.touching= 1
    player.time_since_leftwall=0  
  end
  ...
  def calc_right
    ...
    player.dx = 0
    outputs.sprites << (player.rect.merge path: 'sprites/square/red.png')
    player.touching= 2
    player.time_since_rightwall=0
  end
