...
def defaults args
    ...
    # the number 80 is the size of all the sprites, so that is what is being
    # used to decide the with and height
    args.state.craft_border.x =  760
    args.state.craft_border.y = 400
    args.state.craft_border.w = args.state.sprite_size * 2
    args.state.craft_border.h = args.state.sprite_size * 2
...
def render args  
    ...
    args.outputs.primitives << args.state.inventory_area.map do |a|
        { x: a.x, y: a.y, w: a.w, h: a.h, path: 'sprites/temp/border-black.png' }
    end
